// ============================================================
// AuthProvider — sesión global (fuente única de verdad).
//
// Monta UN solo onAuthStateChanged y carga UNA vez el perfil de
// usuarios/{uid}. Todos los consumidores (useAuth) leen el mismo estado,
// así que no hay ciclos de carga independientes ni parpadeo de rol/teamId.
//
// El contexto y el hook de consumo viven en ./useAuth (archivo aparte para
// mantener limpio el Fast Refresh, igual que AppConfigProvider/useAppConfig).
// ============================================================
...
export function AuthProvider({ children }) {
  ...
  const value = {
    user, role, teamId, empresaId, nombre, movilHabilitado, whatsappBitacora,
    aceptoTerminos, correoPersonal, isOwner: role === 'owner', isVentas: role === 'ventas',
    loading, authError, signIn, signOut, cambiarPassword, aceptarTerminos, guardarCorreoPersonal,
  }
